...
namespace CPMS.Patient.Domain
{
    public class BreachService
    {
        ...
        public virtual int CountPlannedBreachingEvents(int? daysToBreach, BreachFilterInput breachFilterInput, string pathType = null)
        {
            return _plannedEventRepository.Count(GetCriteriaForPlannedBreachingEvents(daysToBreach, breachFilterInput, pathType));
        }
        ...
        public virtual PeriodCounter GetPeriodCounter(int? hospitalId = null, string specialtyCode = null, int? clinicianId = null, string pathwayType = null)
        {
            if (hospitalId == null && specialtyCode == null && clinicianId == null && pathwayType == null) return null;

            var plannedEvents = _plannedEventRepository.Get(
                p => ((hospitalId != null && p.Event.Clinician.Hospital.Id == hospitalId) ||
                     (specialtyCode != null && p.Event.Clinician.Specialty.Code == specialtyCode) ||
                     (clinicianId != null && p.Event.Clinician.Id == clinicianId) ||
                     (pathwayType != null && p.Event.Period.Pathway.PathwayType.ToString() == pathwayType)) &&
                     p.Event.Period.ShouldCountForBreaches && p.Event.Period.IsActive && p.Event.IsActive && p.IsMandatory, null).ToArray();
            ...
            {
                var daysRemaining = @event.GetDaysToBreachAt(_clock.TodayDate);
                if (daysRemaining >= 0 )
                {
        ...
        private Expression<Func<PlannedEvent, bool>> GetCriteriaForPlannedBreachingEvents(int? daysToBreach, BreachFilterInput breachFilterInput, string pathType = null)
        {
            if (daysToBreach < 0)
            {
                // Breached
                return plannedEvent => GetCriteriaForPlannedEvent(plannedEvent, pathType) &&
        ...
        private Expression<Func<PlannedEvent, bool>> GetCriteriaForPeriodPlannedEvents(int periodId, PeriodEventsFilterInput periodEventsFilterInput)
        {
            return plannedEvent => plannedEvent.Event.Period.Id == periodId &&
                                   GetCriteriaForPlannedEvent(plannedEvent, null) &&
                                   GetCriteriaWithFiltersForPeriodPlannedEvents(plannedEvent, periodEventsFilterInput);
        }

        private bool GetCriteriaForPlannedEvent(PlannedEvent plannedEvent, string pathType = null)
        {
            return plannedEvent.Event.IsActive &&
                   plannedEvent.IsMandatory &&
                   plannedEvent.Event.Period.ShouldCountForBreaches &&
                   (string.IsNullOrEmpty(pathType) || plannedEvent.Event.Period.Pathway.PathwayType.ToString() == pathType) &&

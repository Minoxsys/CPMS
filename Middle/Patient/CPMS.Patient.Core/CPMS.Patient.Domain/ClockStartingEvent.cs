...
{
    public class ClockStartingEvent:Event
    {
        ...
        public override void Validate()
        {
                ...
                Code == EventCode.DidNotAttend && !(Period.Pathway.Patient.IsChild(EventDate) || Period.AbleToNotAttendFirstEvent))
            {
                ...
                {
                    Message = string.Format("Event {0}, on {1}, on pathway {2} should not start the clock.", Code.GetDescription(), Period.Name, Period.Pathway.PPINumber),
                    Period = Period
                });
            }
        }

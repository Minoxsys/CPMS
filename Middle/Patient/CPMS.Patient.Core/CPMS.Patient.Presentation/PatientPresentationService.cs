...
namespace CPMS.Patient.Presentation
{
    public class PatientPresentationService
    {
        ...
        {
            return _patientAuthorizationFilter.GetHospitals(role).Select(hospital => _hospitalInfoToHospitalViewModelMapper.Map(hospital));
        }
        ...
        public IEnumerable<PeriodCounterViewModel> GetPeriodCounterForTrust(RoleData role)
        {
            return
                _patientAuthorizationFilter.GetPeriodCounterForTrust(role)
                    .Select(
                        periodCounter => _periodCounterInfoToPeriodCounterViewModelMapper.Map(periodCounter));

        }

        public IEnumerable<PeriodCounterViewModel> GetPeriodCounterForHospital(RoleData role, string hospitalId)
        {
            return
                _patientAuthorizationFilter.GetPeriodCounterForHospital(role, hospitalId)
                    .Select(periodCounter => _periodCounterInfoToPeriodCounterViewModelMapper.Map(periodCounter));
        }

        public IEnumerable<PeriodCounterViewModel> GetPeriodCounterForSpecialty(RoleData role, string specialtyCode, string hospitalId)
        {
            return
                _patientAuthorizationFilter.GetPeriodCounterForSpecialty(role, specialtyCode, hospitalId)
                    .Select(periodCounter => _periodCounterInfoToPeriodCounterViewModelMapper.Map(periodCounter));
        }
    }

...
{
    public class ClockStartingCompletedEvent : CompletedEvent
    {
        ...
        public override void Validate()
        {
                ...
                Name.Code == EventCode.DidNotAttend && !(Period.Pathway.Patient.IsChild(EventDate) || Period.AbleToNotAttendFirstEvent))
            {
                ...
                {
                    Message = string.Format("Completed event {0}, on {1}, on pathway {2} should not start the clock.", Name.Description, Period.Name, Period.Pathway.PPINumber),
                    Period = Period
                });
            }
        }

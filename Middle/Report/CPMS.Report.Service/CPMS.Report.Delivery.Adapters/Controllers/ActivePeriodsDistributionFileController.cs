...
namespace CPMS.Report.Delivery.Adapters.Controllers
{
    public class ActivePeriodsDistributionFileController : ApiController
    {
        ...
        public HttpResponseMessage GetActivePeriodsDistributionReportFile(Format format = Format.Excel, Layout layout = Layout.Tabular, int hospitalId = 0, string specialtyCode = null, int clinicianId = 0, Granularity granularity = Granularity.Hospital)
        {
            var reportFile = _reportService.GetActivePeriodsDistributionReportFile(format, layout, hospitalId, specialtyCode, clinicianId, granularity);

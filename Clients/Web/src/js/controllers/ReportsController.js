...
function ReportsController($scope, $timeout, $http, $window, $location, trustData, menu) {
	...
	function init() {
		...
		that.exportResults = function() {
            ...
            if(that.selectedPathway.id){
                url += "&pathwayType="+that.selectedPathway.id;
            }
			downloadReport(url);
		};
	}
	...
	function initDropdowns() {
		resetPathwayDropdown();
		resetHospitalDropdown();
		resetSpecialtyDropdown();
		resetClinicianDropdown();

		trustData.getPathways(function(pathways) {
			that.pathways = getDefaultPathways().concat(pathways);
			that.selectedPathway = that.pathways[0];
		});

		that.onPathwayDropdownChange = function() {
			resetHospitalDropdown();
			resetSpecialtyDropdown();
			resetClinicianDropdown();

			if (that.selectedPathway.id !== null) {
				trustData.getHospitalsPerPathway(that.selectedPathway.id, function(hospitals) {
		...
		that.onHospitalDropdownChange = function() {
			...
			if (that.selectedHospital.id !== null) {
                console.log(that.selectedHospital.id);
		...
		function resetPathwayDropdown() {
			that.pathways = getDefaultPathways();
			that.selectedPathway = that.pathways[0];
		}
		...
		function getDefaultPathways() {
			return [{ name: "All pathways", id: null }];
		}

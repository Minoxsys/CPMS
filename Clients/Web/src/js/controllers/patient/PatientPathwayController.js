...
app.controller("PatientPathwayController", [ 
	"$scope", "$location", "$sce", "$filter", "$window", 
	"MenuDataProvider", "GridDataProvider", "AjaxHelper", "AuthorizationProvider", "PatientDetailsDisplayType",

	function($scope, $location, $sce, $filter, $window, MenuDataProvider, GridDataProvider, ajax, authProvider, PatientDetailsDisplayType) {
		...
		function getPeriods() {
			...
			ajax("/Patient/api/Periods", {
				...
				$scope.periodValues = promise.data.map(function(item) {
					...
					switch (periodType) {
						case "CancerPeriod": result.name += " (Cancer)"; break;
						case "Non18WeekPeriod": result.name += " (Non 18w)"; break;
						case "PausedPeriod": result.name += " (Paused)"; break;
					}
		...
		function updatePathwayChart() {
			ajax("/Patient/api/PeriodEvents", {
				...
				var events = promise.data.Events.map(function(item) {
					var e = {
						name: item.EventDescription,
					};

...
function ReportsController($scope, $timeout, $http, $window, $location, trustData, filtersTrustData, menu) {
	var that = this;
	...
	function getReportUrl() {
		...
		if (reportType === "periodBreaches") {
			url += "Monthly18wRTTPerformanceFile?";
		...
		url += "&t=" + $window.sessionStorage.accessToken;
	...
	function modifyFiltersMenu() {
		filtersTrustData.onDecorateSpecialty = function(specialty) {
			specialty.onClick = function() {
				var hospitalId = specialty.parentItem.id;

				that.granularity = "specialty";

				trustData.getHospitals(function(hospitals) {
					that.hospitals = getDefaultHospitals().concat(hospitals);
					that.selectedHospital = _.find(hospitals, { id: hospitalId });

					trustData.getSpecialtiesPerHospital(that.selectedHospital.id, function(specialties) {
						that.specialties = getDefaultSpecialties().concat(specialties);
						that.selectedSpecialty = _.find(specialties, { id: specialty.id });

						that.onSpecialtyDropdownChange();
					});
				});
			};
		};
		filtersTrustData.onDecorateClinician = function(clinician) {
			clinician.onClick = function() {
				var specialtyId = clinician.parentItem.id;
				var hospitalId = clinician.parentItem.parentItem.id;

				that.granularity = "clinician";
				...
				trustData.getHospitals(function(hospitals) {
					...
					trustData.getSpecialtiesPerHospital(that.selectedHospital.id, function(specialties) {
						that.specialties = getDefaultSpecialties().concat(specialties);
						that.selectedSpecialty = _.find(specialties, { id: specialtyId });

						trustData.getCliniciansPerSpecialty(that.selectedHospital.id, that.selectedSpecialty.id, function(clinicians) {
							that.clinicians = getDefaultClinicians().concat(clinicians);
							that.selectedClinician = _.find(clinicians, { id: clinician.id });
						});
					});
				});
			};
...
ReportsController.$inject = ["$scope", "$timeout", "$http", "$window", "$location", "TrustDataProvider", "TrustData", "MenuDataProvider"];

...
app.controller("EventBreachesGridController", [
	...
	"BreachesGridDropdowns", "TrustData", "BreachesCountDataProvider", "AuthorizationProvider",

	function($scope, $window, $location, MenuDataProvider, GridDataProvider, SortType, 
			BreachesGridDropdowns, TrustData, breachesCount, authProvider) {
		...
		function initGrid() {
			...
			$scope.gridColumns = {
				...
				lastEvent: 		{ field: "EventDescription" },
			...
			$scope.dataProvider = new GridDataProvider($scope, "/Report/api/EventBreaches");
			...
			$scope.dataProvider.onExtractData = function(data) {
				return {
					data: data.EventBreaches,
					totalCount: data.TotalNumberOfEventBreaches,
				};
			};
			...
			$scope.getPlusMinusClass = function(row) {
				if (row.DelayOrAdvancementDays > 0) {
					return "positive";
				}
				if (row.DelayOrAdvancementDays < 0) {
					return "negative";
		...
		function initFilters() {
			...
			authProvider.getUserInfo().then(function(user) {
				$scope.showDropdowns = user.role.permissions.Trust;

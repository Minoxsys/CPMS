...
app.controller("EventBreachesGridController", [
	...
	function($scope, $window, $location, MenuDataProvider, GridDataProvider, SortType,
		...
		var activePathwayIndex = $location.search().pathwayType.toString();

		var current_menu = initMenu(activePathwayIndex);
		var menuActiveItem;
		var dropdowns;
		...
		function initMenu(pathwayType){
			$scope.pathwayType = pathwayType;
			return MenuDataProvider.tree.categories.breaches[pathwayType.toLowerCase()];
		}
		...
		function initTabs() {
			...
			modifyActiveMenuItem($scope.activeTab, $scope.pathwayType);
		...
		function initGrid() {
			...
			$scope.dataProvider.onAddQueryParams = function(query) {
				...
				if($scope.pathwayType){
					query.pathwayType = $scope.pathwayType;
				}
			...
			$scope.onRowClick = function(row) {
				...
				url += "&pathwayType="+activePathwayIndex;
		...
		function modifyNavigationMenu() {
			...
			for (var i = 0; i < current_menu._item.children.length; i++) {
				var item = current_menu._item.children[i];
		...
		function modifyActiveMenuItem(id) {
			...
			menuActiveItem = current_menu[id]._item;

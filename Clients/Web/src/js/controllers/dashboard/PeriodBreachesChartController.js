...
PeriodBreachesChartController.prototype = {
	...
	getData: function() {
		...
		if (bc.trust) {
			this.dataProvider.getPathwayData(function(data) { 
		...
		if (bc.hospital) {
			this.dataProvider.getHospitalData(bc.id, function(data) {
				for (var i = 0; i < data.items.length; i++) {
		...
		if (bc.specialty) {
			this.dataProvider.getClinicianData(bc.id,bc.hospitalId, function(data) {
				for (var i = 0; i < data.items.length; i++) {

...
function MenuController($scope, $timeout, MenuDataProvider, authProvider) {
	var that = this;
	...
	function handleUserPermissions(user) {
		...
		if (!rights.MonthlyRTTPeriodPerformance &&
			!rights.FuturePeriodBreaches &&
			!rights.ActivePeriodsDistribution) {
			removeMenuItem(tree.categories._item.children, "reports");	
		} else {
			if (!rights.MonthlyRTTPeriodPerformance) {
				removeMenuItem(tree.categories.reports._item.children, "periodBreaches");
			}
			if (!rights.FuturePeriodBreaches) {
				removeMenuItem(tree.categories.reports._item.children, "futurePeriodBreaches");
			}
			if (!rights.ActivePeriodsDistribution) {
				removeMenuItem(tree.categories.reports._item.children, "activePeriods");
			}
		}
...
MenuController.$inject = ["$scope", "$timeout", "MenuDataProvider", "AuthorizationProvider"];

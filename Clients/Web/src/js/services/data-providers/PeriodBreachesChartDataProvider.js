...
PeriodBreachesChartDataProvider.prototype = {
	...
	getData: function(params, callback) {
		...
		this.ajax("/Report/api/PeriodPerformance", params).then(function(promise) {
			var data = {};

			data.items = _.filter(promise.data, function(item) {
					return ((item.AboutToBreachNumber + item.BreachedNumber) > 0);
				})
				.map(function(item) {
					return {
						id: item.Id,
						...
						completed: item.CompletedNumber,
					};
				});

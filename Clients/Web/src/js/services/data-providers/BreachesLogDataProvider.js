...
BreachesLogDataProvider.prototype = {
	...
	getProcessedEventBreachesItem: function(user, item) {
		...
		if (item.Status === "Breached") {
			result.text = printf(" is %s postbreach on event milestone %s.", this.$filter("days")(item.DaysForStatus), item.EventDescription);
		} else
		if (item.Status === "Breach") {
			result.text = printf(" is on breach date on event milestone %s.", item.EventDescription);
		} else
		if (item.Status === "AboutToBreach") {
			result.text = printf(" is %s to breach on event milestone %s.", this.$filter("days")(item.DaysForStatus), item.EventDescription);
		}
